...
export class CrmServiceService {
  ...
  async createOneLeadOnCrm(data: CreateLeadOnCrmDto, moveLead = false) {
    ...
    try {
      ...
      if (moveLead) {
        ...
        if (
          ...
          if (leadRta.code === CodeResponseLeverateEnum.Success) {
            ...
            this.logger.error(
              `[createOneLeadOnCrm] message: ${msg} | leadRta: ${JSON.stringify(
                leadRta,
              )}`,
            );
            await this.builder.getPromiseLeadEventClient(
      ...
      this.logger.error(`[createOneLeadOnCrm] error: ${err.message || err}`);
      return err;
    }
    return null;
  }
  ...
  async taskSaveLeadInCrm(
    ...
    try {
      ...
      this.logger.error(
        `[taskSaveLeadInCrm] Save lead schedulerRegistry: ${
          err.message || err
        }`,
      );
    }
  ...
  async checkLeadDetails(crmType: IntegrationCrmService, lead: LeadInterface) {
    if (!lead.crmTradingPlatformAccountId) {
      ...
      if (
        ...
        this.logger.error(
          `[createOneTransferOnCrm] Can't get the account trading platform id: ${JSON.stringify(
            tpAccountInfo,
          )}`,
        );
    ...
    if (!!data.affiliatesToCheck) {
      ...
      for (const affiliateId of data.affiliatesToCheck) {
        ...
        try {
          ...
          if (leads.length) {
            ...
            for (const leadB2crypto of leadsB2crypto.list) {
              ...
              if (statusesFinded[leadCrm.leadStatusCode]?._id) {
                ...
                this.logger.error(
                  `[checkCrmLeadStatus] No update lead: Status ${leadCrm.leadStatus} not found for ${leadCrm.tpAccount}`,
                );
              }
            }
          }
          ...
          this.logger.error(
            `[checkCrmLeadStatus] Error get lead statuses from CRM ${
              crmType.crm.name
            } - ${affiliateId}: ${error.message || error}`,
          );
        }
      }
      ...
      for (const affiliateId in groupLeadsByAffiliate) {
        ...
        try {
          ...
          this.logger.error(
            `[checkCrmLeadStatus] Error ${error.message || error}`,
          );
          continue;
        }
      }
