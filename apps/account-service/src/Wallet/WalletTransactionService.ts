...
export class WalletTransactionService {
    ...
    private async handleInternalTransfer(createDto: WalletDepositCreateDto, to: any, user: User, host: string) {
        ...
        this.createTransferEvents(to, from, createDto, user, depositWalletCategory, withDrawalWalletCategory, approvedStatus, internalPspAccount, host);
    ...
    private async handleExternalDeposit(createDto: WalletDepositCreateDto, to: any, user: User, host: string) {
        ...
        try {
            ...
            if (!depositAddress) {
                ...
                this.ewalletBuilder.emitAccountEventClient(EventsNamesAccountEnum.updateOne, {
                    id: to._id,
    ...
    private createTransferEvents(to: any, from: any, createDto: WalletDepositCreateDto, user: User, depositWalletCategory: any, withDrawalWalletCategory: any, approvedStatus: any, internalPspAccount: any, host: string) {
        ...
        this.ewalletBuilder.emitTransferEventClient(EventsNamesTransferEnum.createOne, {
            ...commonTransferData,
    ...
    async sweepOmnibus(data: any) {
        ...
        do {
            ...
            for (const from of walletList.list) {
                await this.processSweepWallet(from, walletsBase, valuts, wallets, fireblocksCrm, cryptoType, promises);
            }
    ...
    private async processSweepWallet(
        ...
        if (!walletsBase[from.name]) {
            walletsBase[from.name] = await this.fireblocksService.getWalletBase(fireblocksCrm._id, from.name);
        }
        ...
        if (!valuts[brandId]) {
            valuts[brandId] = await this.getVaultPair(fireblocksCrm._id, walletBase, brandId);
        }

        if (!wallets[brandId]) {
            wallets[brandId] = await this.getWalletPair(walletBase, fireblocksCrm._id, valuts[brandId]);
        }
    ...
    private async getVaultPair(fireblocksCrmId: string, walletBase: any, brandId: string) {
        return {
            deposit: await this.fireblocksService.getVaultBrand(fireblocksCrmId, walletBase, brandId, WalletTypesAccountEnum.VAULT_D),
            withdraw: await this.fireblocksService.getVaultBrand(fireblocksCrmId, walletBase, brandId, WalletTypesAccountEnum.VAULT_W)
        };
    }

    private async getWalletPair(walletBase: any, fireblocksCrmId: string, vaults: any) {
        return {
            deposit: await this.fireblocksService.getWallet(walletBase, fireblocksCrmId, vaults.deposit, WalletTypesAccountEnum.VAULT_D),
            withdraw: await this.fireblocksService.getWallet(walletBase, fireblocksCrmId, vaults.withdraw, WalletTypesAccountEnum.VAULT_W)
        };
    }
    ...
    private async createSweepTransactions(
        ...
        promises.push(
            this.createDepositTransaction(from, vaultFrom, valuts, wallets, valueToDeposit, cryptoType),
            this.createWithdrawTransaction(from, vaultFrom, valuts, wallets, valueToWithdraw, cryptoType)
        );
    }

    private async createDepositTransaction(from: any, vaultFrom: any, valuts: any, wallets: any, value: number, cryptoType: any) {
        return cryptoType
            .createTransaction(
                from.accountId,
                String(value),
            ...
            .catch(err => {
                Logger.error(err, `Sweep deposit error: ${vaultFrom.name}_${from.name}`);
                return null;
            })
            .then(rta => this.updateBalances(from, wallets.deposit, value));
    }

    private async createWithdrawTransaction(from: any, vaultFrom: any, valuts: any, wallets: any, value: number, cryptoType: any) {
        return cryptoType
            .createTransaction(
                from.accountId,
                String(value),
            ...
            .catch(err => {
                Logger.error(err, `Sweep withdrawal error: ${vaultFrom.name}_${from.name}`);
                return null;
            })
