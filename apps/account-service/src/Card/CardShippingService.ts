...
export class CardShippingService {
  ...
  public async getShippingPhysicalCard(
    cardId: string,
    user: User,
  ): Promise<ApiShippingResult> {
  ...
  async shippingPhysicalCard(user: User) {
    ...
    const response = await cardIntegration.shippingPhysicalCard(
      shippingRequest,
    );

    if (response?.data?.id) {
      return await this.createShippingAccount(
        user,
        response as ApiShippingResponse,
      );
    }
  ...
  private async createShippingAccount(
    user: User,
    shippingResponse: ApiShippingResponse,
  ) {
