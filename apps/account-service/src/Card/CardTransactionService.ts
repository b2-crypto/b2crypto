...
export class CardTransactionService {
    ...
    constructor(
        ...
        this.BLOCK_BALANCE_PERCENTAGE = this.configService.get<number>('AUTHORIZATIONS_BLOCK_BALANCE_PERCENTAGE');
    }
    ...
    async rechargeCard(rechargeDto: CardDepositCreateDto, user: User) {
        ...
        const [toAccount, fromAccount] = await Promise.all([
            ...
            this.validateAccount(rechargeDto.from.toString(), TypesAccountEnum.WALLET)
        ]);
        ...
        await Promise.all([
            ...
            this.createTransferRecords(toAccount, fromAccount, rechargeDto.amount, depositCardCategory, withdrawalWalletCategory)
        ]);

        return { ...fromAccount, amount: fromAccount.amount - rechargeDto.amount };
    }

    async processPomeloTransaction(data: PomeloTransactionData): Promise<CardsEnum> {
        try {
            let txnAmount = 0;
            ...
            if (data.authorize) {
                ...
                txnAmount = data.movement?.toUpperCase() === 'DEBIT' ? data.amount * -1 : data.amount;
            }
    ...
    private async validateAccount(accountId: string | Types.ObjectId, expectedType: TypesAccountEnum): Promise<AccountDocument> {
        if (!accountId) {
    ...
    async createCard(createDto: CardCreateDto, user: User) {
        createDto.accountType = createDto.accountType ?? CardTypesAccountEnum.VIRTUAL;
